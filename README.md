...
**docfill** — программа для быстрого заполнения шаблонов документов, написанная на языке **Python**. Подставляет значения из JSON-файлов в файлы (документы) в форматах **DOCX (.docx)**, **ODT (.odt)** и **DOC (.doc)**.
...
**DOCX** и **ODT** поддерживаются напрямую. **DOC** обрабатывается после предварительной конвертации в **DOCX** с помощью ![LibreOffice](https://www.libreoffice.org/).
...
- `--in-place`  
  Подставлять данные (заменять плейсхолдеры) прямо в целевом файле, без создания копий. Недоступно для целевых файлов в формате `DOC`.
