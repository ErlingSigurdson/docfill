...
- `--in-place`  
  Подставлять данные (заменять плейсхолдеры) прямо в целевом файле, без создания копий. Опция недоступна для целевых файлов в формат `DOC`.
...
- `--libreoffice-exec PATH`
  Указать путь к исполняемому файлу LibreOffice или (если имя такого файла глобально доступно из используемой оболочки операционной системы) его имя. Эта опция нужна **только для файлов в формате `DOC`** и **только если программе не удалось самостоятельно найти исполняемый файл LibreOffice**.
